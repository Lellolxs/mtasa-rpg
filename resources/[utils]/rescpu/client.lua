...
addEvent("rescpu:toggle", true);
addEventHandler("rescpu:toggle", root, function(state)
	if (state and not resStat) then
		addEventHandler("onClientRender", root, resStatRender)
		triggerServerEvent("getServerStat", localPlayer)
	elseif (not state and resStat) then 
...
function resStatRender()
	...
	for i, row in ipairs(rows) do
		local percent = row[2]:gsub("%%", "");
		local r, g, b = interpolateBetween(0, 255, 0, 255, 0, 0, tonumber(percent) / 45, "Linear");

		local text = row[1]:sub(0,15)..": "..row[2]
		dxDrawText(text,x+1,y+1,150,15,tocolor(0,0,0,255),1,"default_bold")
		dxDrawText(text,x,y,150,15,tocolor(r, g, b,255),1,"default_bold")
		y = y + 15
	end
