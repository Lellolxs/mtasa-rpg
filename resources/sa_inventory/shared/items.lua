...
local ConstStatusDiffType = (math.random(1, 2) == 1) and 'positive' or 'negative';
local ConstRandomStatusValue = math.random(21, 1274752);
local ItemMaxStatusDiff = 50;
...
Items = {
    ...
    ['m4a4'] = {
        ...
        methods = {
            getDescription = function(item, exactStatus)
                local diff = (ConstStatusDiffType == 'positive')
                            and (ConstRandomStatusValue % ItemMaxStatusDiff) 
                            or -(ConstRandomStatusValue % ItemMaxStatusDiff);

                return "Ez a fegyver még ~" .. math.floor(item.status + diff) .. " lövést bír ki.";
            end, 
        },
    },
